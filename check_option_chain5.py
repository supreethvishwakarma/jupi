...
def check_index(name, prefix, search_exch, spot_exch, spot_token, weekly_1letter=False):
    ...
    if not ltp or not opts:
        print("Missing LTP or contracts -- stopping here.")
        return
    ...
    if not parsed:
        print("Couldn't parse with either pattern -- raw symbols above show the real format to fix.")
        return
    ...
    print(f"Nearest upcoming: {upcoming[0] if upcoming else 'NONE'} "
...
time.sleep(8)
check_index("CRUDEOIL", "CRUDEOIL", "MCX", "MCX", "441732", weekly_1letter=False)
