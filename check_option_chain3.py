...
# quiet down the library's own internal logging (this is what dumped thousands
# of lines last time -- not our own print statements)
...
def check_index(name, prefix, search_exch, spot_exch, spot_token, weekly_1letter=False):
    ...
    if not parsed:
        print("Couldn't parse any -- format differs, need to inspect raw symbols above.")
        return

    codes = sorted(set(p[0] for p in parsed))
    print(f"Distinct expiry codes seen: {codes[:8]}{'...' if len(codes) > 8 else ''}")
    ...
    print(f"Using first expiry code '{codes[0]}' -- ATM strike ~{strike}")
