...
new2 = '''                else:
                    qty_i = pos.get("qty", qty_i)  # use the size locked in at entry, not today's default
                    option_token = pos["option_token"]'''

old3 = '''                        hit_sl = ltp <= pos["sl"] if bull else ltp >= pos["sl"]
                        ...
                        if hit_sl or hit_tg or sqoff:
                            trigger = ("SL" if hit_sl else "Target") if (hit_slor hit_tg) else "Manual"
...
new3 = '''                        hit_sl = ltp <= pos["sl"] if bull else ltp >= pos["sl"]
                        hit_tg = ltp >= pos["target"] if bull else ltp <= pos["target"]
...
old4 = '''            if is_fresh and ts_sig not in seen:
...
new4 = '''            if is_fresh and ts_sig not in seen:
                entry_close = float(chart_df.iloc[-1].close)
...
import shutil
shutil.copy(PATH, PATH + ".bak_vanguardvol")

with open(PATH, "w", encoding="utf-8") as f:
    f.write(content)

print("Patched -- Vanguard now sizes up 2x on elevated volatility, and locks in profit (floor = peak - Rs500) once a trade clears +Rs1,000.")
